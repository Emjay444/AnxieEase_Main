...
-- Admins can view all profiles (if you have admin users)
...
-- Patients can view their assigned psychologist
CREATE POLICY "Patients can view assigned psychologist" ON public.psychologists
  FOR SELECT USING (
    id IN (
      SELECT assigned_psychologist_id FROM public.user_profiles 
      WHERE id = auth.uid()
    )
  );
...
CREATE POLICY "Psychologists can view patient anxiety records" ON public.anxiety_records
  FOR SELECT USING (
    user_id IN (
      ...
      WHERE assigned_psychologist_id IN (
        SELECT id FROM public.psychologists WHERE user_id = auth.uid()
      )
    )
  );
