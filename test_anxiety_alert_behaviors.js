...
if (admin.apps.length === 0) {
  admin.initializeApp({
    ...
    databaseURL: 'https://anxiease-main-default-rtdb.asia-southeast1.firebasedatabase.app'
  });
}
...
async function testSeverityNotifications() {
  ...
  for (let i = 0; i < severityTests.length; i++) {
    ...
    try {
      ...
      console.log(`${test.emoji} Sent ${test.severity} test notification:`, response);
      
      // Add delay between notifications
      await new Promise(resolve => setTimeout(resolve, 2000));
      
    } catch (error) {
      console.error(`❌ Error sending ${test.severity} notification:`, error.message);
    }
  }

  console.log('\n📱 TAP EACH NOTIFICATION TO TEST BEHAVIOR:');
  console.log('─'.repeat(50));
  console.log('1. Tap the 🟢 Mild notification → Should go to notifications screen');
  console.log('2. Tap the 🟡 Moderate notification → Should go to notifications screen'); 
  console.log('3. Tap the 🟠 Severe notification → Should go to notifications screen');
  console.log('4. Tap the 🔴 Critical notification → Should go to notifications screen');
  console.log('');
  ...
  console.log('We can modify main.dart to handle different severities differently:');
