...
async function diagnoseAnxietyDetection() {
  ...
  try {
    ...
    const userSessionsSnapshot = await db.ref(`/users/${REAL_USER_ID}/sessions`).once('value');
    ...
    if (alerts) {
      ...
      console.log("   This suggests the Firebase Cloud Function didn't trigger");
    }
    ...
    if (baseline) {
      ...
      console.log(`   Anxiety threshold: ${Math.round(baseline.heartRate * 1.2)} BPM`);
    } else {
    ...
    const fcmTokenSnapshot = await db.ref(`/users/${REAL_USER_ID}/fcmToken`).once('value');
    ...
    const deviceAssignmentSnapshot = await db.ref('/devices/AnxieEase001/assignment').once('value');
    ...
    if (!sessions) {
      ...
      console.log("⚠️ LIKELY ISSUE: Firebase Cloud Functions not processing the data");
