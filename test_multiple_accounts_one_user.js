...
const FCM_TOKEN =
  "cn2XBAlSTHCrCok-hqkTJy:APA91bHk8LIQFq86JaCHLFv_Sv5P7MPB8WiomcfQzY7lr8fD9zT0hOmKjYjvUUL__7LBQ2LHPgjNSC4-gWDW3FFGviliw9wy7AbaahO6iyTxYOJT63yl7r0"; // Same FCM token (same device)
...
async function testMultipleAccountsOneUser() {
  ...
  try {
    ...
    await db
      .ref(`/users/${PERSONAL_ACCOUNT}/sessions/${sessionId}/metadata`)
      .set({
        deviceId: DEVICE_ID,
        status: "active",
    ...
    console.log(
      "✅ Personal Account: Baseline 70 BPM, FCM token, DEVICE ASSIGNED"
    );
    console.log(
      "✅ Work Account: Baseline 75 BPM, FCM token, NO DEVICE ASSIGNMENT"
    );
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const userDataRef = db
        .ref(`/users/${PERSONAL_ACCOUNT}/sessions/${sessionId}/data`)
        .push();
      ...
      console.log(
        `   📈 ${elapsed}s: ${deviceData.heartRate} BPM → Personal account anxiety check`
      );
    ...
    const personalAlertsSnapshot = await db
      .ref(`/users/${PERSONAL_ACCOUNT}/alerts`)
      .once("value");
    const personalAlerts = personalAlertsSnapshot.val();

    // Check Work Account
    const workAlertsSnapshot = await db
      .ref(`/users/${WORK_ACCOUNT}/alerts`)
      .once("value");
    ...
    if (personalAlerts) {
      ...
      console.log(
        `✅ Personal Account: ${count} anxiety alerts (CORRECT - device assigned)`
      );
    } else {
    ...
    if (workAlerts) {
      ...
      console.log(
        `❌ Work Account: ${count} alerts (WRONG - not assigned to device)`
      );
    } else {
      console.log(
        "✅ Work Account: No alerts (CORRECT - not assigned to device)"
      );
    }
    ...
    console.log(
      "✅ User can switch accounts but only assigned one gets alerts"
    );
