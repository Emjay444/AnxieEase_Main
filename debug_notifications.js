...
async function debugNotificationIssues() {
  ...
  try {
    ...
    console.log(`📱 FCM Token: ${fcmToken ? fcmToken.substring(0, 50) + '...' : 'Not found'}`);
    ...
    if (alertsSnapshot.exists()) {
      ...
      Object.entries(alerts).forEach(([alertId, alert]) => {
        console.log(`   ${alertId}: ${alert.type || 'unknown'} at ${new Date(alert.timestamp).toLocaleString()}`);
      });
    } else {
    ...
    if (assignmentSnapshot.exists()) {
      ...
      console.log(`📅 Assignment time: ${new Date(assignment.assignedAt).toLocaleString()}`);
    ...
    console.log('   • Check Firebase Console > Cloud Messaging for delivery stats');
