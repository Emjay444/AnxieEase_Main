...
async function analyzeNotifications() {
  try {
    ...
    const deletedNotifications = allNotifications.filter(
      (n) => n.deleted_at !== null
    );
    ...
    const duplicates = Object.keys(titleGroups).filter(
    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach((title) => {
        ...
        group.forEach((notif, index) => {
          const time = new Date(notif.created_at).toLocaleString();
          console.log(
            `   ${index + 1}. ID: ${notif.id} | Created: ${time} | Read: ${
              notif.read
            }`
          );
        });
    ...
    if (duplicates.length > 0) {
      recommendations.push(
        `🔄 Remove ${duplicates.reduce(
          (sum, title) => sum + titleGroups[title].length - 1,
          0
    ...
    if (timeGroups["Older"].length > 10) {
      recommendations.push(
        `📅 Archive ${timeGroups["Older"].length} old notifications (older than 1 week)`
      );
    }
    ...
    if (unreadCount > 20) {
      recommendations.push(
        `🔔 Mark ${unreadCount} old unread notifications as read`
      );
    }
    ...
    console.log("═".repeat(50));
    console.log(
      "ℹ️  FCM (Firebase Cloud Messaging) analysis requires checking:"
    );
    ...
    console.log(
      `Active: ${activeNotifications.length} | Deleted: ${deletedNotifications.length}`
    );
    console.log(
      `Unread: ${unreadCount} | Read: ${
        activeNotifications.length - unreadCount
      }`
    );
