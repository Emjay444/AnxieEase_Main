...
async function testBackgroundAnxietyDetection() {
  ...
  try {
    ...
    if (!assignment || assignment.userId !== REAL_USER_ID) {
      throw new Error(`Device ${REAL_DEVICE_ID} is not assigned to user ${REAL_USER_ID}`);
    }
    ...
    console.log("Sending elevated readings for 20 seconds (below 30s threshold)");
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await sendHeartRateData(REAL_USER_ID, sessionId, heartRate);
      console.log(`   📈 Sent: ${heartRate.toFixed(1)} BPM (${(i+1)*2}s elapsed)`);
