...
console.log(`   📘 Elevated:   ${thresholds.elevated} - ${thresholds.mild - 1} BPM (${calculatePercentage(thresholds.elevated, YOUR_BASELINE)}% above baseline)`);
console.log(`   🟡 Mild:       ${thresholds.mild} - ${thresholds.moderate - 1} BPM (${calculatePercentage(thresholds.mild, YOUR_BASELINE)}% above baseline)`);
console.log(`   🟠 Moderate:   ${thresholds.moderate} - ${thresholds.severe - 1} BPM (${calculatePercentage(thresholds.moderate, YOUR_BASELINE)}% above baseline)`);
console.log(`   🔴 Severe:     ${thresholds.severe} - ${thresholds.critical - 1} BPM (${calculatePercentage(thresholds.severe, YOUR_BASELINE)}% above baseline)`);
console.log(`   🚨 Critical:   ${thresholds.critical}+ BPM (${calculatePercentage(thresholds.critical, YOUR_BASELINE)}% above baseline)`);
...
testHeartRates.forEach(hr => {
  ...
  console.log(`${icon} ${hr} BPM → ${severity.toUpperCase()} (${percentage}% above baseline)`);
});
...
console.log('\n🔬 THRESHOLD BREAKDOWN:');
console.log(`• First alert at: ${thresholds.elevated} BPM (+${thresholds.elevated - YOUR_BASELINE} BPM)`);
console.log(`• Anxiety detected at: ${thresholds.mild} BPM (+${thresholds.mild - YOUR_BASELINE} BPM)`);
console.log(`• Emergency level at: ${thresholds.critical} BPM (+${thresholds.critical - YOUR_BASELINE} BPM)`);
...
console.log(`• Your heart rate needs to reach ${thresholds.elevated} BPM or higher`);
