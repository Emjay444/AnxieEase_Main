  ...
  try {
    ...
    if (account1) {
      console.log(
        `   FCM Token: ${
          account1.fcmToken
            ? account1.fcmToken.substring(0, 30) + "..."
            : "NONE"
        }`
      );
      console.log(
        `   Baseline: ${
          account1.baseline ? account1.baseline.heartRate + " BPM" : "Not set"
        }`
      );
      ...
      console.log(
        `   Device Assignment: ${
          assignment && assignment.assignedUser === ACCOUNT_1
            ? "YES ✅"
            : "NO ❌"
        }`
      );
      ...
      const alertsSnapshot = await db
        .ref(`/users/${ACCOUNT_1}/alerts`)
        .once("value");
      const alerts = alertsSnapshot.val();
      console.log(
        `   Recent Alerts: ${alerts ? Object.keys(alerts).length : 0} alerts`
      );
    } else {
    ...
    if (account2) {
      console.log(
        `   FCM Token: ${
          account2.fcmToken
            ? account2.fcmToken.substring(0, 30) + "..."
            : "NONE"
        }`
      );
      console.log(
        `   Baseline: ${
          account2.baseline ? account2.baseline.heartRate + " BPM" : "Not set"
        }`
      );
      ...
      console.log(
        `   Device Assignment: ${
          assignment && assignment.assignedUser === ACCOUNT_2
            ? "YES ✅"
            : "NO ❌"
        }`
      );
    ...
    if (account1 && account2) {
      ...
      if (token1 && token2) {
        if (token1 === token2) {
          ...
          console.log(
            "   Both accounts share the same FCM token (same device)"
          );
          ...
          console.log(
            "✅ DIFFERENT FCM TOKENS - Accounts have separate tokens"
          );
        }
      }
    }
    ...
    console.log(
      "5. This is normal FCM behavior - same device = same notifications"
    );
    ...
    console.log(
      "✅ Only Account 1 triggers anxiety detection (isolation working)"
    );
