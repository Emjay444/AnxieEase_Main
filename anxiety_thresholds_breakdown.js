...
console.log(`Elevated (+10 BPM): ${(yourBaseline + 10).toFixed(1)} BPM (83.9+)`);
console.log(`Mild Anxiety (+15 BPM): ${(yourBaseline + 15).toFixed(1)} BPM (88.9+)`);
console.log(`Moderate (+25 BPM): ${(yourBaseline + 25).toFixed(1)} BPM (98.9+)`);
console.log(`Severe (+35 BPM): ${(yourBaseline + 35).toFixed(1)} BPM (108.9+)`);
console.log(`Critical (+45 BPM): ${(yourBaseline + 45).toFixed(1)} BPM (118.9+)`);
...
console.log('-------------------------------------');
console.log(`Condition: HR ≥ ${(yourBaseline * 1.2).toFixed(1)} BPM (20% above baseline) + Movement < 15/100`);
...
console.log('------------------------------------');
console.log(`Condition: HR ≥ ${(yourBaseline * 1.3).toFixed(1)} BPM (30% above baseline) for 30+ seconds`);
...
console.log('-------------------------------------------');
console.log(`Condition: HR ≥ ${(yourBaseline * 1.2).toFixed(1)} BPM (20% above baseline) for 30+ seconds`);
...
console.log('-------------------------------');
console.log('Condition: Movement > 30/100 + HR increase 20-80% + Gyro < 50/100');
...
const exerciseDetected = currentMovement > 30 && hrPercentageAbove > 0.2 && hrPercentageAbove < 0.8 && currentGyro < 50;
const tremorDetected = currentGyro > 40 && currentMovement > 5 && currentMovement < 30;

console.log('THRESHOLD ANALYSIS:');
console.log('-------------------');
console.log(`✅ Elevated (83.9+ BPM): ${isElevated ? 'YES' : 'NO'}`);
console.log(`🟡 Mild Anxiety (88.9+ BPM): ${isMildAnxiety ? 'YES' : 'NO'}`);  
console.log(`🟠 Moderate Anxiety (96+ BPM): ${isModerateAnxiety ? 'YES' : 'NO'}`);
...
if (currentSpO2 < 90) {
    ...
    console.log('🚨 TREMOR ALERT: Anxiety-related shaking detected (80% confidence)');
} else if (isMildAnxiety && isResting && !exerciseDetected) {
    console.log('🚨 ANXIETY ALERT: High heart rate while resting (85% confidence)');
...
console.log(`   Heart Rate: ≥ ${(yourBaseline + 15).toFixed(1)} BPM (currently ${currentHR})`);
...
console.log(`    Need ~${(88.9 - currentHR).toFixed(1)} more BPM while resting to trigger anxiety alert.`);