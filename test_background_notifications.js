...
async function testBackgroundNotifications() {
  ...
  try {
    ...
    const notificationData = {
      id: `notif_${Date.now()}`,
      title: '🚨 AnxieEase Anxiety Alert',
      message: 'Your heart rate is elevated (92 BPM). Are you feeling anxious or stressed?',
      type: 'anxiety',
      severity: 'mild',
      heartRate: 92,
      baseline: 73.2,
    ...
    const notifRef = await db.ref(`users/${userId}/notifications`).push(notificationData);
    ...
    await db.ref(`devices/AnxieEase001/userNotifications/${userId}`).push(notificationData);
    ...
    const wellnessData = {
      ...
      title: '💚 AnxieEase Wellness Check',
      message: 'How are you feeling today? Take a moment to check your mental wellness.',
      type: 'wellness',
      subtype: 'reminder',
      timestamp: Date.now(),
      created_at: new Date().toISOString(),
    ...
    console.log('⚡ This should trigger the anxiety detection Firebase Function...');
    ...
    console.log('   - Firebase Function should detect HR 94 > 88.2 (mild threshold)');
