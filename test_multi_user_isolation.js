...
async function testMultiUserIsolation() {
  ...
  try {
    ...
    console.log(
      "This should ONLY trigger notifications for User A (assigned user)"
    );
    ...
    for (let i = 0; i < 18; i++) {
      ...
      const userDataRef = db
        .ref(`/users/${USER_A_ID}/sessions/${sessionId}/data`)
        .push();
      ...
      const elapsed = (i + 1) * 3;
      console.log(
        `   📈 ${elapsed}s: ${deviceData.heartRate} BPM → Only User A should be notified`
      );
    ...
    if (userA_alerts) {
      ...
      console.log(
        `✅ User A (assigned): ${alertCount} anxiety alerts generated`
      );
    ...
    if (userB_alerts) {
      ...
      console.log(
        `❌ User B (not assigned): ${alertCount} alerts found - SECURITY ISSUE!`
      );
      console.log(
        "   ⚠️ This should not happen - multi-user isolation failed!"
      );
    } else {
    ...
    console.log("===============================");
    console.log(
      "✅ Only assigned users receive notifications from their device"
    );
