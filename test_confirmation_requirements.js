...
console.log('📋 NEW REQUIREMENT: Mild and Moderate Anxiety Levels ALWAYS Request Confirmation');
console.log('================================================================================\n');
...
console.log(`Mild Level: ${yourBaseline + 15} - ${yourBaseline + 24} BPM (88.9-97.9) → ✅ ALWAYS asks confirmation`);
console.log(`Moderate Level: ${yourBaseline + 25} - ${yourBaseline + 34} BPM (98.9-107.9) → ✅ ALWAYS asks confirmation`);
console.log(`Severe Level: ${yourBaseline + 35}+ BPM (108.9+) → ❌ NO confirmation (immediate alert)`);
console.log(`Critical SpO2/Medical: → ❌ NO confirmation (emergency)\n`);

console.log('📱 TEST SCENARIOS:');
console.log('==================\n');

function testConfirmationScenario(heartRate, movement, gyro, spo2, description) {
  ...
  const exerciseDetected = movement > 30 && (hrElevation / yourBaseline) > 0.2 && (hrElevation / yourBaseline) < 0.8 && gyro < 50;
  ...
  } else if (hrElevation >= 15) { // Above mild threshold
    wouldTrigger = true;
    requiresConfirmation = isMild || isModerate; // NEW: Mild/Moderate always confirm
  ...
  console.log(`   HR: ${heartRate} BPM (+${hrElevation.toFixed(1)} above baseline)`);
  console.log(`   Movement: ${movement}/100, Gyro: ${gyro}/100, SpO2: ${spo2}%`);
  console.log(`   Level: ${isSevere ? 'SEVERE' : isModerate ? 'MODERATE' : isMild ? 'MILD' : 'NORMAL'}`);
  console.log(`   Triggers: ${wouldTrigger ? '🚨 YES' : '✅ NO'}`);
  
  if (wouldTrigger) {
    console.log(`   Confirmation: ${requiresConfirmation ? '❓ ASKS "Are you feeling anxious?"' : '🚨 IMMEDIATE ALERT'}`);
...
console.log('======================');
console.log('✅ Mild anxiety (88.9-97.9 BPM): NOW asks "Are you feeling anxious?"');
console.log('✅ Moderate anxiety (98.9-107.9 BPM): NOW asks "Are you feeling anxious?"');
console.log('❌ Severe anxiety (108.9+ BPM): Still immediate alert (no confirmation)');
console.log('❌ Critical medical (SpO2 <90%): Still immediate alert (emergency)');
