...
async function cleanupNotifications() {
  try {
    ...
    console.log(
      `Found ${allNotifications.length} active notifications to analyze`
    );
    ...
    const duplicateGroups = Object.keys(titleGroups).filter(
    ...
    if (duplicateGroups.length > 0) {
      ...
      for (const title of duplicateGroups) {
        ...
        console.log(
          `   ✅ Keeping: ${new Date(
            toKeep.created_at
          ).toLocaleString()} (ID: ${toKeep.id.substring(0, 8)}...)`
        );
        ...
        for (const notif of toRemove) {
          try {
            ...
            if (deleteError) {
              console.error(
                `   ❌ Failed to remove ${notif.id}: ${deleteError.message}`
              );
            } else {
              console.log(
                `   🗑️  Removed: ${new Date(
                  notif.created_at
                ).toLocaleString()} (ID: ${notif.id.substring(0, 8)}...)`
              );
              removedCount++;
            }
          } catch (error) {
    ...
    const oldUnreadNotifications = allNotifications.filter(
      (notif) =>
        !notif.read &&
      ...
      for (const notif of oldUnreadNotifications) {
        try {
          ...
          if (updateError) {
            ...
            const title =
              notif.title.length > 40
                ? notif.title.substring(0, 40) + "..."
                : notif.title;
    ...
    if (!finalError) {
      ...
      console.log(
        `📊 Final active notifications: ${finalNotifications.length}`
      );
