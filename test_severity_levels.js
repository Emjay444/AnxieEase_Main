...
async function testSeverityLevels() {
  ...
  for (const testCase of testCases) {
    console.log(`${testCase.color} Testing ${testCase.name} severity (${testCase.hr} BPM)`);
    const percentAbove = ((testCase.hr - baselineHR) / baselineHR * 100).toFixed(1);
    console.log(`   📈 ${percentAbove}% above baseline (should be ${testCase.expectedSeverity})`);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      console.log(`   ${testCase.color} ${(i + 1) * 10}s: HR=${testCase.hr} BPM sent`);
      await new Promise(resolve => setTimeout(resolve, 10000)); // Wait 10 seconds
    }
    
    console.log(`   ✅ ${testCase.name} test complete - check for ${testCase.expectedSeverity} notification`);
    console.log("");
