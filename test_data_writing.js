...
async function testDataWriting() {
  ...
  try {
    ...
    console.log(`   Timestamp: ${new Date(testData.timestamp).toLocaleString()}`);
    ...
    if (storedData) {
      ...
      console.log(`✅ Verification successful! Found ${dataKeys.length} data point(s)`);
      
      // Show the data
      dataKeys.forEach(key => {
        const point = storedData[key];
        console.log(`   ${key}: ${point.heartRate} BPM at ${new Date(point.timestamp).toLocaleString()}`);
      });
    } else {
