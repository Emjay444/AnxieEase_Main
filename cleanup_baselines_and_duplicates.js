...
async function cleanupDuplicatesAndBaselines() {
  ...
  try {
    ...
    console.log('✅ Backup created at /system/backups/cleanup_baselines_duplicates');
    console.log('');
    ...
    const duplicateUserIds = userIds.filter((item, index) => userIds.indexOf(item) !== index);
    ...
    console.log(`   Users with duplicate baseline data: ${usersWithDuplicateBaselines}`);
    console.log('');

    // STEP 3: Remove duplicate baseline entries from users node
    console.log('🗑️ Removing duplicate baseline entries from users/[userId]/baseline/...');
    ...
    if (usersData) {
      for (const userId of uniqueUserIds) {
        ...
        if (user && user.baseline !== undefined) {
          ...
          baselinesCleaned++;
          console.log(`   ✅ Removed duplicate baseline for user: ${userId.substring(0, 8)}...`);
        }
      }
    }
    ...
    if (duplicateUserIds.length > 0) {
      ...
      for (const duplicateUserId of duplicateUserIds) {
        console.log(`   Processing duplicate user: ${duplicateUserId.substring(0, 8)}...`);
        ...
        if (userInstances.length > 1) {
          ...
          for (let i = 1; i < userInstances.length; i++) {
            ...
            if (instance.sessions) {
              mergedUser.sessions = { ...(mergedUser.sessions || {}), ...instance.sessions };
            }
            
            // Merge alerts
            if (instance.alerts) {
              mergedUser.alerts = { ...(mergedUser.alerts || {}), ...instance.alerts };
            }
            
            // Merge anxiety_alerts
            if (instance.anxiety_alerts) {
              mergedUser.anxiety_alerts = { ...(mergedUser.anxiety_alerts || {}), ...instance.anxiety_alerts };
            }
          ...
          console.log(`   ✅ Merged duplicate entries for user: ${duplicateUserId.substring(0, 8)}...`);
        }
      }
    }
    ...
    if (finalUserBaselinesData) {
      ...
      console.log(`   Sample baseline structure for ${sampleUserId.substring(0, 8)}...:`);
    ...
    if (finalUsersData) {
      ...
      console.log(`   Remaining duplicate baselines: ${remainingDuplicateBaselines}`);
      console.log(`   user_baselines entries: ${finalUserBaselinesData ? Object.keys(finalUserBaselinesData).length : 0}`);
    }
    ...
    console.log('🔒 Backup is available for recovery at /system/backups/cleanup_baselines_duplicates');
    throw error;
  }
}
