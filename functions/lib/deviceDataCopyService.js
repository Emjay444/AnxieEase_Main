...
async function verifySupabaseAdmin(accessToken) {
    ...
    try {
        ...
        const adminResp = await fetchImpl(`${SUPABASE_URL}/rest/v1/admin_profiles?id=eq.${uid}&select=id`, {
            headers: {
                apikey: SUPABASE_SERVICE_ROLE_KEY,
    ...
    try {
        ...
        if (action === "assign") {
            ...
            if (priorAssignmentSnapshot.exists()) {
                const priorAssignment = priorAssignmentSnapshot.val();
                if (priorAssignment.assignedUser &&
                    priorAssignment.assignedUser !== userId) {
                    if (priorAssignment.activeSessionId) {
                        await db
                            .ref(`/users/${priorAssignment.assignedUser}/sessions/${priorAssignment.activeSessionId}/metadata`)
                            .update({
                    ...
                    console.log(`🧹 Closed prior session and cleared stale current reading for ${priorAssignment.assignedUser} before reassigning to ${userId}`);
                }
            }
