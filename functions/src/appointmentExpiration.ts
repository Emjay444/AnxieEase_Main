 ...
 */
export const checkExpiredAppointments = database
  .ref("/trigger_appointment_expiration")
  .onWrite(async (change: any, context: any) => {
    ...
    try {
      ...
      logger.info(
        `✅ Appointment expiration check completed. Expired: ${result.expiredCount}, Errors: ${result.errorCount}`
      );
      return result;
    } catch (error: any) {
      logger.error(
        "❌ Error in scheduled appointment expiration check:",
        error
      );
      throw error;
    }
 ...
 */
export const expireAppointmentsNow = database
  .ref("/manual_expire_appointments")
  .onWrite(async (change: any, context: any) => {
    ...
    try {
      const result = await expireOverdueAppointments();
      logger.info(
        `✅ Manual appointment expiration completed. Expired: ${result.expiredCount}, Errors: ${result.errorCount}`
      );
      return {
        success: true,
        ...result,
...
async function expireOverdueAppointments(): Promise<{
  ...
  logger.info(
    `🔍 Checking for appointments created before: ${cutoffTime.toISOString()}`
  );
  ...
  try {
    ...
    logger.info(
      `📋 Found ${pendingAppointments.length} appointments to expire`
    );
    ...
    for (const appointment of pendingAppointments) {
      try {
        logger.info(
          `⏰ Expiring appointment ${appointment.id} (created: ${appointment.created_at})`
        );
        ...
        if (updateError) {
          ...
          try {
            ...
            logger.warn(
              `⚠️ Failed to create expiration notification for appointment ${appointment.id}:`,
              notifError
            );
...
async function createExpirationNotification(appointment: any): Promise<void> {
  try {
    const { error } = await supabase.from("notifications").insert({
      ...
      message: `Your appointment request from ${new Date(
        appointment.created_at
      ).toLocaleDateString()} has expired after 24 hours. Please submit a new request if you still need an appointment.`,
      type: "info",
    ...
    if (error) {
      logger.warn(
        `⚠️ Failed to create expiration notification for user ${appointment.user_id}:`,
        error
      );
    } else {
      logger.info(
...
export async function triggerAppointmentExpiration(): Promise<void> {
  ...
  try {
    ...
    logger.error(
      "❌ Manual trigger: Error triggering appointment expiration:",
      error
    );
    throw error;
  }
