...
/**
 * SMART DEVICE DATA SYNC - No Redundancy Version
 * 
 * This replaces the redundant copying system with a smart approach:
 * - Respects device native format (YYYY_MM_DD_HH_MM_SS) - NEVER removes
 * - Only copies to user sessions (individual tracking)
 * - Does NOT create timestamp duplicates in device history
 * - Preserves anxiety detection capabilities
 */
...
interface SensorData {
  ...
  battPerc: number;
  worn: number;
  timestamp: string; // Device uses string format like "2025-10-05 20:13:51"
  accelX?: number;
  accelY?: number;
  accelZ?: number;
  gyroX?: number;
 ...
 */
export const smartDeviceDataSync = functions.database
  ...
  .onWrite(async (change, context) => {
    ...
    try {
      ...
      const toNativeKeyFromString = (s: string): string | null => {
        // Already native format
        if (/^\d{4}_\d{2}_\d{2}_\d{2}_\d{2}_\d{2}$/.test(s)) return s;
        // Common device format with dashes/colons and space separator
        const m = s.match(
          /^(\d{4})[-_/](\d{2})[-_/](\d{2})[ T](\d{2})[:_](\d{2})[:_](\d{2})$/
        );
        if (m) return `${m[1]}_${m[2]}_${m[3]}_${m[4]}_${m[5]}_${m[6]}`;
      ...
      const toNativeKeyFromMillis = (millis: number): string => {
      ...
      const sessionHistoryKey =
        (typeof currentData.timestamp === 'string'
      ...
      const enrichedHistoryData = {
        ...
        sessionId: activeSessionId,
        userId: assignedUser,
        source: "device_current_sync", // Track source for debugging
      };
 ...
 */
export const disabledDeviceHistoryCreation = functions.database
 ...
 */
export const removeTimestampDuplicates = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        `Smart cleanup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
 ...
 */
export const monitorDuplicationPrevention = functions.pubsub
