  ...
  try {
    ...
    if (userSnapshot.exists()) {
      ...
      if (userData.baseline) {
        ...
        console.log(
          `   Device: ${userData.baseline.deviceId || "Not specified"}`
        );
        console.log(
          `   Set: ${new Date(userData.baseline.timestamp).toLocaleString()}`
        );
        ...
        if (userData.alerts) {
          ...
          if (recentAlerts.length > 0) {
            console.log(
              `Found ${recentAlerts.length} alerts in last 24 hours:`
            );

            recentAlerts.slice(0, 5).forEach((alert, index) => {
              const alertHR = alert.heartRate;
              const percentAbove = (
                ((alertHR - baselineHR) / baselineHR) *
                100
              ).toFixed(1);
              ...
              console.log(
                `   ${
                  index + 1
                }. ${time}: ${alertHR} BPM (+${percentAbove}% above baseline)`
              );
    ...
    if (currentSnapshot.exists()) {
      ...
      if (userData && userData.baseline) {
        ...
        console.log(
          `   Difference: ${percentDiff > 0 ? "+" : ""}${percentDiff}%`
        );
    ...
    console.log(
      "Looking for movement/accelerometer data in current sensor readings..."
    );
    ...
    if (currentSnapshot.exists()) {
      ...
      if (hasMovementData) {
        ...
        console.log(
          "❌ No movement/accelerometer data found in current readings"
        );
    ...
    if (userData && userData.baseline) {
      console.log(
        `   ✅ User baseline exists: ${userData.baseline.heartRate} BPM`
      );
    ...
    console.log(
      "   ❓ Current algorithm may not filter exercise-induced HR spikes"
    );

    console.log("\n3. 🚨 ALERT ACCURACY:");
    console.log(
      "   ⚠️  Without movement filtering, exercise could trigger false alarms"
    );
