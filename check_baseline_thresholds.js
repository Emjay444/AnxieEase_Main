...
async function checkThresholds() {
  ...
  try {
    ...
    for (const [userId, userData] of Object.entries(users)) {
      ...
      console.log(`📡 Assigned Device: ${userData.assignedDevice || '❌ None'}`);
      ...
      if (userData.baseline) {
        ...
        const critical = userData.baseline * 1.50;  // 50% above baseline
        
        console.log(`📊 Calculated thresholds:`);
        console.log(`   🟢 Mild: ${Math.round(mild)} BPM (${userData.baseline} + 15%)`);
        console.log(`   🟡 Moderate: ${Math.round(moderate)} BPM (${userData.baseline} + 25%)`);
        console.log(`   🟠 Severe: ${Math.round(severe)} BPM (${userData.baseline} + 35%)`);
        console.log(`   🔴 Critical: ${Math.round(critical)} BPM (${userData.baseline} + 50%)`);
        ...
        for (const testHR of testRates) {
          ...
          const trigger = severity !== 'normal' ? '🚨 TRIGGERS' : '⭕ No trigger';
    ...
    if (devices) {
      for (const [deviceId, deviceData] of Object.entries(devices)) {
        if (deviceData.current) {
          console.log(`\n📡 Device: ${deviceId}`);
          console.log(`💓 Current HR: ${deviceData.current.heartRate || 'N/A'} BPM`);
          console.log(`⏰ Last updated: ${new Date(deviceData.current.timestamp || 0).toLocaleString()}`);
        }
      }
    }
      ...
      if (userData.assignedDevice) {
        ...
        if (sessions) {
          ...
          for (const [sessionId, sessionData] of Object.entries(sessions)) {
            console.log(`   Session ${sessionId}: ${sessionData.heartRate || 'N/A'} BPM at ${new Date(sessionData.timestamp || 0).toLocaleString()}`);
          }
        } else {
