...
console.log('\n📋 SYSTEM OVERVIEW:');
console.log('• Your baseline heart rate: 73.2 BPM (from Supabase device assignment)');
...
console.log(`   📘 Elevated:   ${thresholds.elevated} BPM (+10 BPM, ${((10/baseline)*100).toFixed(1)}%)`);
console.log(`   🟡 Mild:       ${thresholds.mild} BPM (+15 BPM, ${((15/baseline)*100).toFixed(1)}%)`);
console.log(`   🟠 Moderate:   ${thresholds.moderate} BPM (+25 BPM, ${((25/baseline)*100).toFixed(1)}%)`);
console.log(`   🔴 Severe:     ${thresholds.severe} BPM (+35 BPM, ${((35/baseline)*100).toFixed(1)}%)`);
console.log(`   🚨 Critical:   ${thresholds.critical} BPM (+45 BPM, ${((45/baseline)*100).toFixed(1)}%)`);
...
const examples = [
  { hr: 85, level: 'elevated', msg: 'Heart Rate Elevated - Take a moment to breathe' },
  { hr: 90, level: 'mild', msg: 'Mild Anxiety Detected - Try breathing exercises' },
  { hr: 100, level: 'moderate', msg: 'Moderate Anxiety Alert - Consider grounding techniques' },
  { hr: 110, level: 'severe', msg: 'High Anxiety Detected - Use your coping strategies' },
  { hr: 120, level: 'critical', msg: 'Critical Alert - Seek immediate support if needed' }
];

examples.forEach(ex => {
  const icon = {elevated: '📘', mild: '🟡', moderate: '🟠', severe: '🔴', critical: '🚨'}[ex.level];
