...
async function findBaseline() {
  ...
  try {
    ...
    const baseline1 = await db.ref(`/users/${REAL_USER_ID}/baselines/${DEVICE_ID}`).once('value');
    if (baseline1.exists()) {
    ...
    const baseline2 = await db.ref(`/users/${REAL_USER_ID}/baseline`).once('value');
    if (baseline2.exists()) {
    ...
    const baseline3 = await db.ref(`/users/${REAL_USER_ID}/profile/baseline`).once('value');
    if (baseline3.exists()) {
    ...
    const baseline4 = await db.ref(`/devices/${DEVICE_ID}/baseline`).once('value');
    if (baseline4.exists()) {
