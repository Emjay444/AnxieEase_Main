...
async function investigateFCMTokenLeak() {
  ...
  try {
    ...
    const userA_tokenSnapshot = await db
      .ref(`/users/${USER_A_ID}/fcmToken`)
      .once("value");
    const userA_token = userA_tokenSnapshot.val();

    const userB_tokenSnapshot = await db
      .ref(`/users/${USER_B_ID}/fcmToken`)
      .once("value");
    const userB_token = userB_tokenSnapshot.val();

    console.log("User A FCM Token:");
    console.log(
      userA_token ? `  ${userA_token.substring(0, 30)}...` : "  No token found"
    );

    console.log("User B FCM Token:");
    console.log(
      userB_token ? `  ${userB_token.substring(0, 30)}...` : "  No token found"
    );
    ...
    if (userA_token && userB_token && userA_token === userB_token) {
      ...
      console.log(
        "   This explains why User B received User A's notifications"
      );
      console.log(
        "   Solution: Each user must have their own unique FCM token"
      );
    } else if (userA_token && userB_token) {
      console.log(
        "✅ Users have different FCM tokens - investigating further..."
      );
    }
    ...
    if (assignment) {
      ...
      if (assignment.assignedUser === USER_A_ID) {
        console.log(
          "✅ Assignment is correct - User A should get notifications"
        );
      } else {
    ...
    const userA_alertsSnapshot = await db
      .ref(`/users/${USER_A_ID}/alerts`)
      .once("value");
    const userA_alerts = userA_alertsSnapshot.val();

    const userB_alertsSnapshot = await db
      .ref(`/users/${USER_B_ID}/alerts`)
      .once("value");
    ...
    console.log(
      "2. TOKEN SHARING: App not properly isolating FCM tokens by user"
    );
    console.log(
      "3. FLUTTER APP ISSUE: App showing notifications for wrong user"
    );

    console.log("\n🔧 IMMEDIATE FIXES NEEDED:");
    console.log("1. Each user must have their own unique FCM token");
    console.log(
      "2. Flutter app must register separate tokens per user account"
    );
    ...
    if (userA_token && userB_token && userA_token === userB_token) {
      console.log("\n🚨 ROOT CAUSE FOUND:");
      console.log(
        "Both users have the same FCM token, so notifications go to the same device!"
      );
      ...
      console.log(
        "2. Implement proper FCM token isolation per user account, OR"
      );
