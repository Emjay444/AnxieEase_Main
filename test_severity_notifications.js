...
async function testSeverityNotifications() {
  try {
    ...
    const severityTests = [
      {
        ...
        message: 'Your heart rate is slightly elevated. Consider taking a short break and trying some breathing exercises.',
        icon: '💛',
        priority: 'normal'
      },
      {
        ...
        message: 'Your anxiety levels are moderately elevated. It would be good to practice some calming techniques now.',
        icon: '🧡',
        priority: 'high'
      },
      {
        ...
        message: 'Your anxiety levels are significantly elevated. Please take immediate action to calm yourself and consider contacting support.',
        icon: '❤️',
        priority: 'high'
      },
      {
        ...
        message: 'Your anxiety levels are critically high. Please seek immediate assistance and contact your healthcare provider if symptoms persist.',
        icon: '🚨',
        priority: 'max'
      }
    ];
    ...
    for (let i = 0; i < severityTests.length; i++) {
      ...
      console.log(`${i + 1}. Testing ${test.level.toUpperCase()} severity notification...`);
      ...
      try {
        ...
        if (notificationError) {
          console.error(`❌ Failed to store ${test.level} notification:`, notificationError.message);
          continue;
        }
        ...
        const fcmResponse = await fetch('https://fcm.googleapis.com/fcm/send', {
          method: 'POST',
          headers: {
            'Authorization': 'key=AAAAdaxno_4:APA91bFGBBUF3Qq8KrNEpKrV1dYEXSWkB5xRo4g8yF9X8UYbJ1dRa3w5jLOLj1f8ptvMlMsYh8Vbr2QyZYbGG5Y0oQ7HK9xF9wF1b8rA1d4U9LpA6I8a7F0lJ2mQ0S4', // AnxieEase FCM server key
          ...
          body: JSON.stringify({
            ...
            android: {
              ...
              notification: {
                ...
                visibility: 'public',
                icon: 'notification_icon',
                color: test.level === 'critical' ? '#E53E3E' : test.level === 'severe' ? '#F56500' : test.level === 'moderate' ? '#D69E2E' : '#38A169'
              }
            }
          })
        });
        ...
        if (!fcmResponse.ok) {
          ...
          if (fcmResult.success === 1) {
            console.log(`   ✅ FCM ${test.level} notification sent successfully`);
          } else {
            console.log(`   ⚠️  FCM ${test.level} notification sent with issues:`, fcmResult);
          }
        }

        console.log(`   📱 ${test.level.charAt(0).toUpperCase() + test.level.slice(1)} alert sent!\n`);
        ...
        console.error(`❌ Error sending ${test.level} notification:`, error.message);
      }
    }
    ...
    if (fetchError) {
      console.error('❌ Failed to verify stored notifications:', fetchError.message);
    } else {
