...
async function checkDeviceNotificationSettings() {
  ...
  try {
    ...
    console.log(
      "   1. ⚙️  Go to: Settings > Apps > AnxieEase (or your app name)"
    );
    ...
    console.log(
      "   • For testing, fully CLOSE the app (swipe away from recent apps)"
    );
    console.log(
      "   • Background notifications work better when app is fully closed"
    );
  } catch (error) {
    console.error("❌ Error:", error.message);
    if (error.code === "messaging/registration-token-not-registered") {
      console.log(
        "🔄 Token may be expired - restart the app to get a fresh token"
      );
