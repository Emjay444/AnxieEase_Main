...
async function targetedUserCleanup() {
  ...
  try {
    ...
    if (activeUserSnapshot.exists()) {
      ...
      console.log(`   Baseline: ${activeUserData.baseline ? activeUserData.baseline.heartRate + ' BPM' : 'None'}`);
      console.log(`   Sessions: ${activeUserData.sessions ? Object.keys(activeUserData.sessions).length : 0}`);
      console.log(`   Source: ${activeUserData.baseline ? activeUserData.baseline.source : 'Unknown'}`);
    ...
    for (const userToRemove of USERS_TO_REMOVE) {
      try {
        ...
        if (userSnapshot.exists()) {
          ...
          console.log(`   Had baseline: ${userData.baseline ? userData.baseline.heartRate + ' BPM' : 'None'}`);
          console.log(`   Had alerts: ${userData.alerts ? Object.keys(userData.alerts).length : 0}`);
          console.log(`   Had sessions: ${userData.sessions ? Object.keys(userData.sessions).length : 0}`);
    ...
    console.log("✅ Device assignment: Preserved");
    console.log(`✅ Active baseline: ${cleanupResults.dataPreserved.activeBaseline ? 'Preserved' : 'Missing'}`);
    console.log(`✅ Current sessions: ${cleanupResults.dataPreserved.currentSession ? 'Preserved' : 'Missing'}`);
