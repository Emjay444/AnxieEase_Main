...
async function safeFirebaseCleanup() {
  ...
  try {
    ...
    if (historySnapshot.exists()) {
      ...
      console.log(`   ✅ Removed ${oldHistoryCount} old history records (>7 days)`);
      ...
      try {
        ...
        if (sessionsSnapshot.exists()) {
          ...
          if (oldSessionsCount > 0) {
            console.log(`   ✅ Removed ${oldSessionsCount} old sessions for user ${userId}`);
            cleanupResults.oldSessionsRemoved += oldSessionsCount;
          }
        }
      } catch (error) {
        console.log(`   ❌ Error cleaning sessions for ${userId}: ${error.message}`);
      }
    }
    ...
    for (const userId of REAL_USERS) {
      ...
      if (userSnapshot.exists()) {
        ...
        console.log(`      Baseline: ${userData.baseline ? userData.baseline.heartRate + ' BPM' : 'None'}`);
        console.log(`      FCM Token: ${userData.fcmToken ? 'Present' : 'Missing'}`);
        console.log(`      Alerts: ${userData.alerts ? Object.keys(userData.alerts).length : 0} alerts`);
      } else {
    ...
    console.log(`✅ Old history records removed: ${cleanupResults.oldHistoryRemoved}`);
    console.log(`✅ Old sessions removed: ${cleanupResults.oldSessionsRemoved}`);
