  ...
  async testSustainedAnxietyDetection() {
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const dataRef = db.ref(`/users/${testUserId}/sessions/${sessionId}/data`).push();
        await dataRef.set(normalData);
        
        console.log(`📊 Normal HR sent: ${normalData.heartRate} BPM (${i + 1}/3)`);
      ...
      console.log('⏰ This will take ~30 seconds to trigger sustained detection...\n');
      ...
      for (let i = 0; i < 4; i++) {
        ...
        console.log(`🚨 Elevated HR sent: ${elevatedData.heartRate} BPM (${(elevatedData.heartRate - 70) / 70 * 100}% above baseline) - ${i + 1}/4`);
      ...
      console.log('Device assignment:', assignmentSnapshot.exists() ? '✅ Present' : '❌ Missing');
      ...
      console.log('User baseline:', baselineSnapshot.exists() ? baselineSnapshot.val() + ' BPM' : 'Missing');
