...
async function testNotificationFlow() {
  ...
  try {
    ...
    if (!deviceInfo) {
      console.log(
        "❌ CRITICAL: No device info found - notifications will fail!"
      );
      return;
    }
    ...
    if (!baseline) {
      console.log(
        "❌ WARNING: No baseline found - anxiety detection may not work"
      );
      return;
    }
    ...
    if (fcmTokenSnapshot.exists()) {
      ...
      console.log(
        "⚠️  No FCM token found - app needs to register for notifications"
      );
    ...
    console.log(
      `Level: ${isModerateLevel ? "MODERATE" : isMildLevel ? "MILD" : "NORMAL"}`
    );

    if (isMildLevel || isModerateLevel) {
      console.log(
        "🚨 WOULD TRIGGER: Anxiety detection with confirmation request"
      );
      console.log(
        '📱 Notification would be sent asking: "Are you feeling anxious?"'
      );
    } else {
    ...
    if (allGood) {
      ...
      console.log(
        "2. Increase your heart rate to 89+ BPM (light exercise, then sit)"
      );
      console.log('3. Watch for notification: "Are you feeling anxious?"');
      console.log(
        "4. Check both system notifications and in-app notifications"
      );
    } else {
