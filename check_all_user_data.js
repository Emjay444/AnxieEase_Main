...
async function checkAllUserData() {
  ...
  try {
    ...
    const userDataSnapshot = await db.ref(`/users/${REAL_USER_ID}`).once('value');
    ...
    if (userData) {
      ...
      if (userData.sessions) {
        ...
        sessionKeys.forEach(sessionKey => {
          ...
          console.log(`       Status: ${session.metadata?.status || 'unknown'}`);
          console.log(`       Device: ${session.metadata?.deviceId || 'unknown'}`);
    ...
    const deviceSnapshot = await db.ref('/devices/AnxieEase001/assignment').once('value');
    ...
    if (assignment) {
      ...
      console.log(`   Match: ${assignment.userId === REAL_USER_ID ? '✅' : '❌'}`);
    } else {
    ...
    if (assignment && assignment.sessionId) {
      ...
      const sessionDataSnapshot = await db.ref(`/users/${REAL_USER_ID}/sessions/${assignment.sessionId}`).once('value');
      const sessionData = sessionDataSnapshot.val();
      
      if (sessionData) {
        console.log("✅ Session data exists:");
        console.log(`   Metadata: ${sessionData.metadata ? '✅' : '❌'}`);
        console.log(`   Data: ${sessionData.data ? Object.keys(sessionData.data).length + ' points' : '❌ No data'}`);
      } else {
