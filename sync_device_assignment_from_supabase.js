...
async function syncDeviceAssignmentFromSupabase() {
  ...
  try {
    ...
    const currentAssignmentSnapshot = await db.ref(`/devices/${DEVICE_ID}/assignment`).once('value');
    ...
    if (currentAssignment) {
      ...
      console.log(`   Assigned At: ${new Date(currentAssignment.assignedAt).toLocaleString()}`);
    } else {
    ...
    await db.ref(`/users/${NEW_USER_ID}/sessions/${NEW_SESSION_ID}/metadata`).set({
      deviceId: DEVICE_ID,
      status: "active",
    ...
    const verifySnapshot = await db.ref(`/devices/${DEVICE_ID}/assignment`).once('value');
    ...
    console.log(`Match: ${verifiedAssignment.assignedUser === NEW_USER_ID ? '✅ YES' : '❌ NO'}`);
