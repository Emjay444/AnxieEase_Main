  ...
  try {
    ...
    const testNotifications = [
      ...
      },
      {
        ...
        message: 'Great job this week! You completed 5 breathing exercises and 3 wellness check-ins.',
        type: 'log',
        icon: '📈'
      },
      {
    ...
    for (let i = 0; i < testNotifications.length; i++) {
      ...
      console.log(`${i + 1}. Adding "${notification.title.replace(/[^\w\s]/gi, '')}" notification...`);
      ...
      try {
          ...
          .insert([
            {
              ...
              related_screen: notification.type === 'reminder' ? 'breathing_screen' : 'home_screen',
              related_id: null
            }
          ])
          .select()
        ...
        if (notificationError) {
          console.error(`❌ Failed to store notification:`, notificationError.message);
          continue;
        }

        console.log(`   ✅ Stored "${notification.type}" notification successfully`);
    ...
    if (fetchError) {
      ...
      allNotifications.forEach((notif, index) => {
        ...
        console.log(`   ${index + 1}. [${notif.type.toUpperCase()}] ${notif.title.replace(/[^\w\s]/gi, '')} (${time})`);
      });
    }

    console.log('\n📱 Now check your AnxieEase app:');
    console.log('   • Homepage "Recent Notifications" section should show new cards');
    console.log('   • Tap "See All" to view the notification screen');
    console.log('   • Different notification types should have different colors');
    ...
    console.log('   ✅ Improved in-app banner popups (when new notifications arrive)');
