...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imdxc3VzdGp4emp6Zm50Y3NudnBrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDEyMDg4NTgsImV4cCI6MjA1Njc4NDg1OH0.RCS_0fSVYnYVY2qr0Ow1__vBC4WRaVg_2SDatKREVHA';
  ...
  try {
    ...
    if (notifications.length > 0) {
      ...
      notifications.slice(0, 5).forEach((notif, index) => {
        ...
        console.log(`${index + 1}. ${notif.title} - ${notif.type} (${createdAt})`);
      });
    }
    ...
    console.log('─'.repeat(40));
    console.log('❌ FCM handler in Flutter app is not calling _storeAnxietyAlertNotification()');
    console.log('❌ _storeAnxietyAlertNotification() is failing silently');
    console.log('❌ User authentication issue in Flutter (required for createNotification)');
    
    console.log('\n💡 NEXT STEPS:');
    console.log('─'.repeat(40));
    console.log('1. Check Flutter debug console when you receive FCM notifications');
    ...
    console.log('3. If no messages appear, the FCM handler condition might not be matching');
