...
async function checkDeviceData() {
  ...
  try {
    ...
    if (deviceData) {
      ...
      console.log(`   Movement: accelX=${deviceData.accelX}, accelY=${deviceData.accelY}, accelZ=${deviceData.accelZ}`);
      console.log(`   Gyroscope: gyroX=${deviceData.gyroX}, gyroY=${deviceData.gyroY}, gyroZ=${deviceData.gyroZ}`);
      console.log(`   Timestamp: ${new Date(deviceData.timestamp).toLocaleString()}`);
      console.log(`   Data Age: ${Math.floor((Date.now() - deviceData.timestamp) / 1000)} seconds ago\n`);
    } else {
    ...
    if (assignment && assignment.userId) {
      ...
      if (userData) {
        ...
        console.log(`   Notifications Enabled: ${userData.notificationsEnabled ? '✅ Yes' : '❌ No'}\n`);
      } else {
      ...
      const notifSnapshot = await notifRef.orderByChild('timestamp').limitToLast(5).once('value');
      ...
      if (notifications) {
        Object.entries(notifications)
          ...
          .forEach(([id, notif]) => {
            console.log(`   ${new Date(notif.timestamp).toLocaleString()}: ${notif.message}`);
          });
      } else {
