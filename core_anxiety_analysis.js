...
async function analyzeCoreQuestions() {
  ...
  try {
    ...
    const currentSnapshot = await db.ref('/devices/AnxieEase001/current').once('value');
    ...
    if (userData && userData.baseline) {
      ...
      const threshold = Math.round(baselineHR * 1.20); // +20% typically used
      const percentAbove = ((currentHR - baselineHR) / baselineHR * 100).toFixed(1);
    ...
    const movementFields = sensorFields.filter(field => 
  ...
  console.log("   • Current device data shows no accelerometer/movement fields");
