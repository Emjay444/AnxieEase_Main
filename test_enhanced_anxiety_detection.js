...
async function testEnhancedAnxietyDetection() {
  ...
  try {
    ...
    console.log(`   Heart Rate: ${realData.heartRate} BPM (baseline: ${baseline})`);
    console.log(`   SpO2: ${realData.spo2}%`);
    console.log(`   Accelerometer: X=${realData.accelX}, Y=${realData.accelY}, Z=${realData.accelZ}`);
    console.log(`   Gyroscope: X=${realData.gyroX}, Y=${realData.gyroY}, Z=${realData.gyroZ}`);
    
    // Calculate movement intensity (same logic as in functions)
    const accelMagnitude = Math.sqrt(realData.accelX * realData.accelX + realData.accelY * realData.accelY + realData.accelZ * realData.accelZ);
    const movementIntensity = Math.min(100, Math.abs(accelMagnitude - 9.8) * 10);
    
    const gyroMagnitude = Math.sqrt(realData.gyroX * realData.gyroX + realData.gyroY * realData.gyroY + realData.gyroZ * realData.gyroZ);
    ...
    console.log(`   HR above baseline: +${(realData.heartRate - baseline).toFixed(1)} BPM`);
    ...
    const exerciseDetected = sustainedMovement && moderateHRIncrease && steadyActivity;
    ...
    const restingAnxiety = realData.heartRate > baseline * 1.2 && movementIntensity < 15;
    
    console.log('\nDetection results:');
    console.log(`   ✅ Exercise detected: ${exerciseDetected ? 'YES' : 'NO'}`);
    console.log(`   🤲 Tremor detected: ${tremorDetected ? 'YES' : 'NO'}`);
    console.log(`   😰 Resting anxiety: ${restingAnxiety ? 'YES - WOULD TRIGGER ALERT' : 'NO'}`);
    ...
    const exerciseMovement = Math.min(100, Math.abs(Math.sqrt(exerciseData.accelX * exerciseData.accelX + exerciseData.accelY * exerciseData.accelY + exerciseData.accelZ * exerciseData.accelZ) - 9.8) * 10);
    const exerciseGyro = Math.min(100, Math.sqrt(exerciseData.gyroX * exerciseData.gyroX + exerciseData.gyroY * exerciseData.gyroY + exerciseData.gyroZ * exerciseData.gyroZ) * 100);
    
    console.log(`Input: HR=${exerciseData.heartRate} BPM, Movement=${exerciseMovement.toFixed(1)}, Gyro=${exerciseGyro.toFixed(1)}`);
    
    const exerciseHRElevation = (exerciseData.heartRate - baseline) / baseline;
    const exercisePattern = exerciseMovement > 30 && exerciseHRElevation > 0.2 && exerciseHRElevation < 0.8 && exerciseGyro < 50;
    
    console.log(`Exercise pattern detected: ${exercisePattern ? 'YES - NO ALERT' : 'NO - WOULD ALERT'}`);
    ...
    const tremorMovement = Math.min(100, Math.abs(Math.sqrt(tremorData.accelX * tremorData.accelX + tremorData.accelY * tremorData.accelY + tremorData.accelZ * tremorData.accelZ) - 9.8) * 10);
    const tremorGyroActivity = Math.min(100, Math.sqrt(tremorData.gyroX * tremorData.gyroX + tremorData.gyroY * tremorData.gyroY + tremorData.gyroZ * tremorData.gyroZ) * 100);
    
    console.log(`Input: HR=${tremorData.heartRate} BPM, Movement=${tremorMovement.toFixed(1)}, Gyro=${tremorGyroActivity.toFixed(1)}`);
    
    const tremorPattern = tremorGyroActivity > 40 && tremorMovement > 5 && tremorMovement < 30;
    
    console.log(`Tremor pattern detected: ${tremorPattern ? 'YES - HIGH CONFIDENCE ALERT' : 'NO'}`);
