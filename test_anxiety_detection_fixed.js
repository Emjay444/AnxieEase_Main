...
const USER_FCM_TOKEN = "cn2XBAlSTHCrCok-hqkTJy:APA91bHk8LIQFq86JaCHLFv_Sv5P7MPB8WiomcfQzY7lr8fD9zT0hOmKjYjvUUL__7LBQ2LHPgjNSC4-gWDW3FFGviliw9wy7AbaahO6iyTxYOJT63yl7r0";
...
async function testAnxietyDetectionFixed() {
  ...
  try {
    ...
    const assignmentSnapshot = await db.ref(`/devices/${REAL_DEVICE_ID}/assignment`).once('value');
    ...
    for (let i = 0; i < 22; i++) {
      ...
      // Write to Firebase
      const dataRef = db.ref(`/users/${REAL_USER_ID}/sessions/${sessionId}/data`).push();
      await dataRef.set(testData);
      
      dataPoints.push(testData);
      
      const elapsed = (i + 1) * 2;
      console.log(`   📈 ${elapsed}s: ${testData.heartRate} BPM (${testData.heartRate >= 84 ? 'ELEVATED' : 'normal'})`);
      
      if (i < 21) { // Don't wait after the last one
    ...
    const alertsSnapshot = await db.ref(`/users/${REAL_USER_ID}/alerts`).once('value');
    ...
    if (alerts) {
      ...
      const dataSnapshot = await db.ref(`/users/${REAL_USER_ID}/sessions/${sessionId}/data`).once('value');
