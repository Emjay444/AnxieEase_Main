...
async function testCompleteDataFlow() {
  ...
  try {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const userDataRef = db.ref(`/users/${REAL_USER_ID}/sessions/${sessionId}/data`).push();
      ...
      console.log(`   📈 ${elapsed}s: ${deviceData.heartRate} BPM (device + user data)`);
    ...
    if (alerts) {
      ...
      const sessionDataSnapshot = await db.ref(`/users/${REAL_USER_ID}/sessions/${sessionId}/data`).once('value');
      ...
      const deviceCurrentSnapshot = await db.ref(`/devices/${REAL_DEVICE_ID}/current`).once('value');
