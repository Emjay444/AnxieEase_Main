    ...
    for (const userId of SIMILAR_USERS) {
      ...
      if (userSnapshot.exists()) {
        ...
        if (userData.baseline) {
          ...
          console.log(
            `   Device ID: ${userData.baseline.deviceId || "Not specified"}`
          );
          console.log(
            `   Timestamp: ${new Date(
              userData.baseline.timestamp
            ).toLocaleString()}`
          );
        } else {
        ...
        if (userData.alerts) {
          ...
          const alertTimestamps = Object.keys(userData.alerts).map(
            (key) => userData.alerts[key].timestamp || 0
          );
