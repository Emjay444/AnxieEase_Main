...
console.log(`📊 Anxiety Trigger: ${sustainedThreshold.toFixed(1)} BPM (20% above baseline)`);
console.log(`   - Must stay at/above this level for 30+ seconds`);
console.log(`   - Calculates: ${YOUR_BASELINE} × 1.2 = ${sustainedThreshold.toFixed(1)} BPM`);
...
testHRs.forEach(hr => {
  ...
  const icon = {mild: '🟡', moderate: '🟠', severe: '🔴', critical: '🚨'}[severity];
...
console.log('\n✅ TRIGGER CONDITIONS:');
console.log(`• Heart rate ≥ ${sustainedThreshold.toFixed(1)} BPM for 30+ seconds`);
...
const scenarios = [
  { hr: 90, duration: 35, severity: 'mild', description: 'Basic anxiety detection' },
  { hr: 100, duration: 45, severity: 'moderate', description: 'Moderate anxiety alert' },
  { hr: 115, duration: 60, severity: 'severe', description: 'Severe anxiety warning' },
  { hr: 130, duration: 30, severity: 'critical', description: 'Emergency level alert' }
];

scenarios.forEach(scenario => {
  const percentage = ((scenario.hr - YOUR_BASELINE) / YOUR_BASELINE * 100).toFixed(1);
  const icon = {mild: '🟡', moderate: '🟠', severe: '🔴', critical: '🚨'}[scenario.severity];
  console.log(`${icon} ${scenario.hr} BPM for ${scenario.duration}s → ${scenario.description} (${percentage}% above)`);
});
...
console.log('\n📊 SUMMARY:');
console.log(`Your sustained anxiety detection triggers at ${sustainedThreshold.toFixed(1)} BPM`);
