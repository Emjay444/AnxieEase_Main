...
async function checkNotificationStatus() {
  ...
  try {
    ...
    const notificationsSnapshot = await notificationsRef.orderByChild('timestamp').limitToLast(5).once('value');
    ...
    if (notifications) {
      Object.entries(notifications)
        ...
        .forEach(([id, notif]) => {
          console.log(`   ${new Date(notif.timestamp).toLocaleString()}: ${notif.title}`);
      ...
      console.log('   ❌ No notifications found in users/{userId}/notifications');
    }
    
    // Path 2: Anxiety alerts
    const alertsRef = db.ref(`users/${userId}/anxietyAlerts`);
    const alertsSnapshot = await alertsRef.orderByChild('timestamp').limitToLast(3).once('value');
    ...
    if (alerts) {
      Object.entries(alerts)
        .sort(([,a], [,b]) => b.timestamp - a.timestamp)
        .forEach(([id, alert]) => {
          console.log(`   ${new Date(alert.timestamp).toLocaleString()}: HR ${alert.heartRate} BPM`);
        });
    } else {
    ...
    if (deviceData) {
      ...
      console.log(`   Timestamp: ${new Date(deviceData.timestamp).toLocaleString()}`);
      console.log(`   Age: ${Math.floor((Date.now() - deviceData.timestamp) / 1000)} seconds ago`);
      ...
      if (deviceData.heartRate >= mildThreshold) {
        console.log(`   🚨 SHOULD TRIGGER: ${deviceData.heartRate} >= ${mildThreshold} (mild anxiety)`);
      } else {
        console.log(`   ✅ Normal: ${deviceData.heartRate} < ${mildThreshold} (threshold)`);
      }
    }
    
    // Check if Firebase Functions triggered
    console.log('\n⚡ Checking Firebase Functions logs...');
    console.log('   (Note: Check Firebase console for real-time function execution logs)');
    
    console.log('\n📊 NOTIFICATION TEST RESULTS:');
    console.log('==============================');
    console.log(notifications ? '✅ Notifications saved to database' : '❌ No notifications in database');
    console.log(alerts ? '✅ Anxiety alerts saved' : '❌ No anxiety alerts saved');
    console.log(deviceData ? '✅ Device data updated' : '❌ No device data found');
