...
async function sendDatabaseCompatibleNotifications() {
  ...
  console.log('📋 These notifications will be stored in Supabase AND show in your app screens!');
  console.log('─'.repeat(60));
  ...
  for (let i = 0; i < severityTests.length; i++) {
    ...
    const percentageAbove = Math.round(((test.heartRate - test.baseline) / test.baseline) * 100);
    ...
    try {
      ...
      console.log(`✅ Sent ${test.severity} alert (HR: ${test.heartRate}, +${percentageAbove}%):`, response);
  ...
  console.log('• Check notification screen → Should see in "All" and "Alerts" filters');  
  console.log('• Tap any notification → Should navigate to notifications screen');
