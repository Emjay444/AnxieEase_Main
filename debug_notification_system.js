  ...
  try {
    ...
    for (const userId of userIds) {
      ...
      if (user.assignedDevice) {
        usersWithDevices++;
        console.log(
          `📡 User ${userId}: Assigned to device ${user.assignedDevice}`
        );
      }
    }
    ...
    console.log(`\n📊 Summary:`);
    console.log(
      `- Users with FCM tokens: ${usersWithTokens}/${userIds.length}`
    );
    console.log(
      `- Users with assigned devices: ${usersWithDevices}/${userIds.length}`
    );
    ...
    for (const deviceId of deviceIds) {
      ...
      if (device.current && device.current.heartRate) {
        console.log(
          `💓 Device ${deviceId}: Current HR = ${device.current.heartRate} BPM`
        );
      }
    }
