...
async function fixDeviceAssignment() {
  ...
  try {
    ...
    console.log('3. 🔄 The app should automatically update the FCM token in Firebase');
    console.log('4. 📬 You should start receiving notifications');
    console.log('5. ⚠️  If still no notifications, check device notification permissions');

    console.log('\n🔍 HOW TO FIND YOUR REAL USER ID:');
    console.log('=================================');
    console.log('In your Flutter app, add this debug code to see your user ID:');
    console.log('```dart');
    console.log('final user = Supabase.instance.client.auth.currentUser;');
    console.log('debugPrint("Your User ID: ${user?.id}");');
    console.log('```');
    console.log('Then replace the userId in this script with your real ID and run again.');
    ...
    console.log('✓ Check Firebase console for FCM token under /users/yourId/fcmToken');
    console.log('✓ Verify device assignment under /devices/AnxieEase001/userId');
