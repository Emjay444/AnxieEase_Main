...
class FixedAnxietyDetectionTester {
  ...
  async testFixedSustainedDetection() {
    ...
    try {
      ...
      const userSessionDataRef = db.ref(`/users/${testUserId}/sessions/${sessionId}/data`);
      ...
      console.log('📊 Adding historical data points with 35+ second sustained elevation:');
      for (const dataPoint of historyDataPoints) {
        ...
        console.log(`   ${secondsAgo}s ago: ${dataPoint.heartRate} BPM ${status}`);
      }
      
      console.log('✅ User session history established with sustained elevated period');
      ...
      console.log(`🚨 Current data: ${currentElevatedData.heartRate} BPM (ELEVATED - should complete 30+ second sustained period)`);
      ...
      if (alerts) {
        ...
        if (userCurrent) {
          console.log('✅ Data reached user session:', userCurrent.heartRate, 'BPM');
          console.log('💡 The detection may take a few more seconds to process...');
        } else {
          console.log('❌ Data not reaching user session - check Cloud Functions');
        }
      }
      ...
      console.log('✅ User session data points:', sessionData ? Object.keys(sessionData).length : 0);
      
      if (sessionData) {
        const dataPoints = Object.values(sessionData).sort((a, b) => a.timestamp - b.timestamp);
        console.log('📊 Chronological heart rate data:');
        dataPoints.slice(-4).forEach((point, index) => {
          const status = point.heartRate >= 84 ? '🚨' : '✅';
          console.log(`   ${status} ${point.heartRate} BPM at ${new Date(point.timestamp).toLocaleTimeString()}`);
        });
      }
