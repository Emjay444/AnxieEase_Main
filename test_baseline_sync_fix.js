...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFhdm1rb2dyd3Zjend2eG5zcGhvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjYyODg4MDEsImV4cCI6MjA0MTg2NDgwMX0.qNn7KLjzKTZt8a8_jcB2W3VWGBYXGJNOkGT3r0cRrE8';
...
async function testBaselineSync() {
    ...
    try {
        ...
        for (const device of devices) {
            ...
            console.log(`Device ${device.device_id} → User ${device.user_id.substring(0, 8)}...`);
        ...
        if (testDevice) {
            ...
            const userBaseline = baselines.find(b => b.user_id === testDevice.user_id);
            
            if (userBaseline) {
                console.log(`📋 Manual Sync Test:`);
                console.log(`   Current Device Baseline: ${testDevice.baseline_hr} BPM`);
                console.log(`   User's Actual Baseline: ${userBaseline.baseline_hr} BPM`);
                
                if (testDevice.baseline_hr !== userBaseline.baseline_hr) {
                    console.log(`   ⚠️  Would update device baseline to: ${userBaseline.baseline_hr} BPM`);
                    console.log(`   📝 SQL: UPDATE wearable_devices SET baseline_hr = ${userBaseline.baseline_hr} WHERE device_id = '${testDevice.device_id}';`);
                } else {
        ...
        console.log('\n🔧 NEXT STEPS:');
        console.log('===============');
        console.log('1. Run the SQL from fix_baseline_sync_supabase.sql in Supabase SQL Editor');
        ...
        console.log('4. Verify baseline automatically updates to match user profile\n');
