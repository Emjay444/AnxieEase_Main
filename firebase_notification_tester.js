...
const ANXIETY_PATTERNS = {
  mild: {
    ...
    hrRange: [Math.round(BASELINE_HR * 1.2), Math.round(BASELINE_HR * 1.29)], // ~106-114 BPM
    color: "🟡",
  },
  moderate: {
    name: "Moderate Anxiety",
    description: "30-49% above baseline (88.2)",
    hrRange: [Math.round(BASELINE_HR * 1.3), Math.round(BASELINE_HR * 1.49)], // ~115-131 BPM
    color: "🟠",
  },
  severe: {
    name: "Severe Anxiety",
    description: "50-79% above baseline (88.2)",
    hrRange: [Math.round(BASELINE_HR * 1.5), Math.round(BASELINE_HR * 1.79)], // ~132-158 BPM
    color: "🔴",
  },
  critical: {
    name: "Critical Anxiety",
    description: "80%+ above baseline (88.2)",
    hrRange: [Math.round(BASELINE_HR * 1.8), Math.round(BASELINE_HR * 1.95)], // ~159-172 BPM
    color: "🚨",
  },
  normal: {
    name: "Normal Range",
...
async function verifyExistingEnvironment() {
  ...
  try {
    ...
    if (assignment && assignment.assignedUser) {
      console.log(
        `✅ Device ${DEVICE_ID} is assigned to user: ${assignment.assignedUser}`
      );
      ...
      if (assignment.fcmToken) {
        console.log(
          `✅ FCM token found: ${assignment.fcmToken.substring(0, 20)}...`
        );
        console.log(
          `✅ Token last refreshed: ${assignment.lastTokenRefresh || "Unknown"}`
        );
      } else {
        ...
        console.log(
          "   3. The app will automatically generate and store FCM token"
        );
        console.log("   4. Check Firebase Console to see the token appear");
        console.log("   5. Then close the app and run this test again");
        console.log("");
        console.log(
          "💡 Without FCM token, anxiety alerts won't be sent when app is closed"
        );
        console.log("");
      }
    } else {
...
async function sendSensorData(anxietyLevel) {
  ...
  try {
    ...
    await db
      .ref(`/users/${REAL_USER_ID}/sessions/${REAL_SESSION_ID}/current`)
      .set(sensorData);

    // Add to user session history with timestamp key (matches your structure)
    const now = new Date();
    const historyKey = `${now.getFullYear()}_${String(
      now.getMonth() + 1
    ...
    await db
      .ref(
        `/users/${REAL_USER_ID}/sessions/${REAL_SESSION_ID}/history/${historyKey}`
      )
      .set(historyData);

    // Update session metadata
    await db
      .ref(`/users/${REAL_USER_ID}/sessions/${REAL_SESSION_ID}/metadata`)
      .update({
    ...
    const percentageAbove = Math.round(
      ((heartRate - BASELINE_HR) / BASELINE_HR) * 100
    );

    console.log(
      `${pattern.color} ${pattern.name} | HR: ${Math.round(
        heartRate
      )} BPM (+${percentageAbove}%) | ${new Date().toLocaleTimeString()}`
    );
  } catch (error) {
...
function startSending(anxietyLevel) {
  ...
  console.log(
    `📊 HR Range: ${pattern.hrRange[0]}-${pattern.hrRange[1]} BPM (${pattern.description})`
  );
...
function showMenu() {
  ...
  console.log(
    `Device: ${DEVICE_ID} | User: ${REAL_USER_ID} | Baseline: ${BASELINE_HR} BPM\n`
  );

  Object.entries(ANXIETY_PATTERNS).forEach(([key, pattern]) => {
    const [minHR, maxHR] = pattern.hrRange;
    console.log(
      `${key.padEnd(8)} - ${pattern.color} ${
        pattern.name
      } (${minHR}-${maxHR} BPM)`
    );
  });
      ...
      switch (command) {
        case "start":
          ...
          if (!level || !ANXIETY_PATTERNS[level]) {
            console.log(
              "❌ Invalid anxiety level. Use: mild, moderate, severe, critical, or normal"
            );
          } else {
            startSending(level);
          }
