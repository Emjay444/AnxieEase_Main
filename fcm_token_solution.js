...
async function generateFCMTokenSolution() {
  ...
  const userA_tokenSnapshot = await db
    .ref("/users/5efad7d4-3dcd-4333-ba4b-41f86/fcmToken")
    .once("value");
  const tokenA = userA_tokenSnapshot.val();

  const userB_tokenSnapshot = await db
    .ref("/users/test-user-b-not-assigned/fcmToken")
    .once("value");
  ...
  if (tokenA && tokenB && tokenA === tokenB) {
    ...
    console.log(
      "But if same device, notifications still appear for both users in app."
    );
  }
}
