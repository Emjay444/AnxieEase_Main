...
async function checkUserBaseline() {
  try {
    ...
    const baselineRef = admin.database().ref(`/users/${userId}/profile/baseline`);
    ...
    if (snapshot.exists()) {
      ...
      console.log(`   Calculated At: ${baseline.calculatedAt ? new Date(baseline.calculatedAt).toLocaleString() : 'Not set'}`);
      console.log(`   Sample Count: ${baseline.sampleCount || 'Not set'}`);
      console.log(`   Confidence: ${baseline.confidence || 'Not set'}%`);
    } else {
      console.log('❌ User does not have baseline - will create one with 73.2 BPM');
    }
