...
async function monitorFirebaseChanges() {
  ...
  console.log("=============================");
  console.log(
    "This script will watch Firebase for changes from your Supabase webhook"
  );
  ...
  const isCurrentlyInSync =
    initialAssignment.assignedUser === "5efad7d4-3dcd-4333-ba4b-4f68c14a4f86";
  ...
  deviceAssignmentRef.on("value", (snapshot) => {
    ...
    if (assignment.supabaseSync) {
      ...
      console.log(
        `      Synced At: ${new Date(
          assignment.supabaseSync.syncedAt
        ).toLocaleString()}`
      );
      console.log(
        `      Baseline HR: ${assignment.supabaseSync.baselineHR} BPM`
      );
      console.log(
        `      Webhook Trigger: ${assignment.supabaseSync.webhookTrigger}`
      );
    }
  ...
  setTimeout(() => {
    console.log("\n⏰ Timeout after 2 minutes");
    console.log(
      "If webhook didn't trigger, there might be a configuration issue"
    );
