...
async function backupDatabase() {
  ...
  try {
    ...
    console.log(
      "✅ Backup created successfully at /system/backups/redundancy_removal_backup"
    );
    console.log(`📊 Backup contains:`);
    console.log(
      `   - device_assignments: ${
        deviceAssignmentsData ? Object.keys(deviceAssignmentsData).length : 0
      } devices`
    );
    console.log(
      `   - sensorData: ${
        sensorDataData ? Object.keys(sensorDataData).length : 0
      } devices`
    );
    console.log(
      `   - devices_metadata: ${
        Object.keys(backup.devices_metadata).length
      } devices`
    );
...
async function verifyCleanup() {
  ...
  try {
    ...
    console.log(
      `   - device_assignments exists: ${deviceAssignmentsSnapshot.exists()}`
    );
...
async function removeRedundancy() {
  ...
  console.log(
    "⚠️  This will remove redundant nodes: device_assignments, sensorData"
  );
  ...
  try {
    ...
    console.log(
      "🔒 Backup available at: /system/backups/redundancy_removal_backup"
    );
  } catch (error) {
