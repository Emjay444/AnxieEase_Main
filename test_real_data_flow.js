...
async function testRealDataFlow() {
  ...
  try {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const elapsed = (i + 1) * 2;
      console.log(`   📈 ${elapsed}s: ${deviceData.heartRate} BPM → /devices/${REAL_DEVICE_ID}/current`);
    ...
    console.log("The realTimeSustainedAnxietyDetection function should trigger...");
    await sleep(10000);
    ...
    const currentSnapshot = await db.ref(`/devices/${REAL_DEVICE_ID}/current`).once('value');
