...
class MainActivity : FlutterActivity() {
    ...
    private lateinit var bluetoothMonitorChannel: MethodChannel
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        iotEventChannel.setStreamHandler(object : EventChannel.StreamHandler {
            override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
                ...
                BluetoothIoTGatewayService.setEventSink(events)
            }
            
            override fun onCancel(arguments: Any?) {
                Log.d(TAG, "📡 EventChannel: Flutter stopped listening for IoT data")
                iotEventSink = null
                BluetoothIoTGatewayService.setEventSink(null)
            }
        })
        ...
        // Setup legacy bluetooth_monitor channel for backward compatibility
        bluetoothMonitorChannel = MethodChannel(
            flutterEngine.dartExecutor.binaryMessenger,
            "bluetooth_monitor"
        )
        
        bluetoothMonitorChannel.setMethodCallHandler { call, result ->
            Log.d(TAG, "📞 BluetoothMonitor call received: ${call.method}")
            ...
            when (call.method) {
                "startBluetoothService" -> {
                    ...
                    Log.d(TAG, "🚀 Starting Bluetooth service: device=$deviceAddress, user=$userId")
                    
                    if (deviceAddress != null && userId != null) {
                        // Forward to the new IoT Gateway service
                        val success = startIoTGatewayService(deviceAddress, userId, "AnxieEase001")
                        result.success(success)
                    } else {
                        Log.e(TAG, "❌ Missing required parameters for startBluetoothService")
                        result.error("INVALID_ARGS", "Missing deviceAddress or userId", null)
                    }
                }
                ...
                "isServiceRunning" -> {
                    ...
                    val isRunning = BluetoothIoTGatewayService.isServiceRunning()
                    Log.d(TAG, "🔍 Service running status: $isRunning")
                    result.success(isRunning)
                }
                
                else -> {
                    Log.w(TAG, "⚠️ Unknown bluetooth_monitor method: ${call.method}")
                    result.notImplemented()
                }
            }
        }
