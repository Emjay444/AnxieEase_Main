...
class DeviceAssignmentTester {
  ...
  async testAssignment() {
    ...
    try {
      ...
      const userSessionRef = db.ref(`/users/${testUserId}/sessions/${sessionId}/metadata`);
      ...
      const userCurrentRef = db.ref(`/users/${testUserId}/sessions/${sessionId}/current`);
      ...
      if (userCurrentData) {
        ...
        console.log('✅ Cloud Function copyDeviceCurrentToUserSession is working!');
      } else {
        console.log('⚠️ Data not found in user session - Cloud Function might not be deployed');
      }
      ...
      console.log('📱 Watch for anxiety detection notification in your Cloud Function logs!');
      ...
      if (alerts) {
        ...
        console.log('✅ realTimeSustainedAnxietyDetection Cloud Function is working!');
      } else {
        console.log('⚠️ No anxiety alerts found - either threshold not met or sustained detection needs more time');
      }
      ...
      const historySnapshot = await deviceHistoryRef.limitToLast(1).once('value');
      const historyData = historySnapshot.val();
      
      if (historyData) {
        console.log('✅ Device history found:', Object.keys(historyData).length, 'entries');
      } else {
        console.log('⚠️ No device history found');
      }
      
      // Check user session data
      const userDataRef = db.ref(`/users/${testUserId}/sessions/${sessionId}/data`);
      const userDataSnapshot = await userDataRef.limitToLast(1).once('value');
      const userData = userDataSnapshot.val();
      
      if (userData) {
        console.log('✅ User session data found:', Object.keys(userData).length, 'entries');
      } else {
      ...
      console.log(userCurrentData ? '✅' : '⚠️', 'Data copying to user session:', userCurrentData ? 'Working' : 'Check Cloud Functions');
      console.log(alerts ? '✅' : '⚠️', 'Anxiety detection:', alerts ? 'Working' : 'Needs more sustained data');
