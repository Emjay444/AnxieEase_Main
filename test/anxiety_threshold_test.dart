  ...
  group('🧪 Anxiety Detection Threshold & Notification Tests', () {
    ...
    group('🚨 High Confidence Detection Tests', () {
      ...
      test('should trigger for severely elevated heart rate alone', () {
        ...
        print('   HR: 125.0 BPM (${((125.0 / 70.0 - 1) * 100).toStringAsFixed(1)}% above baseline)');
        print('   SpO2: 97.0% (Normal)');
        print('   Movement: 0.5 (Moderate)');
        print('   Confidence: ${(result.confidenceLevel * 100).toStringAsFixed(1)}%');
        print('   Action: ${result.confidenceLevel >= 0.8 ? '⚡ IMMEDIATE' : '🔔 NOTIFICATION'}');
        print('   Reason: "${result.reason}"');
      });
    });
    ...
    group('⚠️ Medium Confidence Tests (User Confirmation)', () {
      test('should request user confirmation for moderate elevation', () {
        ...
        print('   HR: 88.0 BPM (${((88.0 / 70.0 - 1) * 100).toStringAsFixed(1)}% above baseline)');
        ...
        final isModerateConfidence = result.confidenceLevel >= 0.6 && result.confidenceLevel < 0.8;
        print('   Action: ${isModerateConfidence ? '❓ REQUEST USER CONFIRMATION' : result.confidenceLevel >= 0.8 ? '⚡ IMMEDIATE ALERT' : '👀 CONTINUE MONITORING'}');
        print('   Reason: "${result.reason}"');
    ...
    group('✅ Normal Metrics Tests (No Detection)', () {
      test('should not trigger on normal physiological variation', () {
        ...
        expect(result.confidenceLevel, lessThan(0.6)); // Low confidence = no action
        
        print('😌 NORMAL PHYSIOLOGICAL STATE:');
        print('   HR: 75.0 BPM (${((75.0 / 70.0 - 1) * 100).toStringAsFixed(1)}% above baseline - normal variation)');
    ...
    group('🎯 Boundary Condition Tests', () {
      test('should test exact threshold boundaries', () {
        ...
        for (final testCase in testCases) {
          ...
          print('   Metrics: HR ${testCase['hr']}, SpO2 ${testCase['spo2']}%, Movement ${testCase['movement']}');
          print('   Detected: ${result.triggered}');
          print('   Confidence: ${(result.confidenceLevel * 100).toStringAsFixed(1)}%');
          print('   Abnormal: HR=${result.abnormalMetrics['heartRate']}, SpO2=${result.abnormalMetrics['spO2']}');
    ...
    group('📱 Notification System Simulation', () {
      test('should demonstrate complete notification flow', () {
        ...
        for (final scenario in scenarios) {
          ...
          print('   📊 HR: ${scenario['hr']}, SpO2: ${scenario['spo2']}%, Movement: ${scenario['movement']}');
          print('   🎯 Detection: ${result.triggered}');
          print('   📈 Confidence: ${(result.confidenceLevel * 100).toStringAsFixed(1)}%');
          ...
          if (result.triggered) {
            ...
            print('   ⚙️  Requires Confirmation: ${result.requiresUserConfirmation}');
          }
        }
      });
    });
    ...
    group('🔄 Real-World Anxiety Episode Simulation', () {
      test('should demonstrate complete anxiety attack progression', () {
        ...
        final timeline = [
          {'time': '10:00 AM', 'hr': 74.0, 'spo2': 98.0, 'move': 0.1, 'context': 'Studying calmly'},
          {'time': '10:30 AM', 'hr': 82.0, 'spo2': 97.0, 'move': 0.3, 'context': 'Reading difficult material'},
          {'time': '11:00 AM', 'hr': 89.0, 'spo2': 95.0, 'move': 0.5, 'context': 'Anxiety starting to build'},
          {'time': '11:15 AM', 'hr': 96.0, 'spo2': 94.0, 'move': 0.7, 'context': 'Panic attack beginning'},
          {'time': '11:20 AM', 'hr': 105.0, 'spo2': 92.0, 'move': 0.8, 'context': 'Full panic attack'},
          {'time': '11:30 AM', 'hr': 88.0, 'spo2': 95.0, 'move': 0.4, 'context': 'Starting to calm down'},
          {'time': '11:45 AM', 'hr': 76.0, 'spo2': 97.0, 'move': 0.2, 'context': 'Recovery phase'},
          {'time': '12:00 PM', 'hr': 74.0, 'spo2': 98.0, 'move': 0.1, 'context': 'Back to baseline'},
        ];
        ...
        for (int i = 0; i < timeline.length; i++) {
          ...
          print('   📊 HR: ${point['hr']} BPM (${hrPercent >= 0 ? '+' : ''}${hrPercent.toStringAsFixed(1)}% from baseline)');
          print('   🫁 SpO2: ${point['spo2']}% ${(point['spo2'] as double) <= 94 ? '(CRITICAL)' : (point['spo2'] as double) <= 95 ? '(LOW)' : '(NORMAL)'}');
          print('   🏃 Movement: ${point['move']} ${(point['move'] as double) > 0.6 ? '(HIGH)' : (point['move'] as double) > 0.3 ? '(MODERATE)' : '(LOW)'}');
          ...
          if (i == 4) { // Peak of panic attack
            expect(result.triggered, isTrue, reason: 'Should detect anxiety at panic attack peak');
            expect(result.confidenceLevel, greaterThan(0.7), reason: 'Should have high confidence during panic attack');
          }
          
          // Validate normal state at baseline
          if (i == 0 || i == 7) { // Calm states
            expect(result.confidenceLevel, lessThan(0.6), reason: 'Should have low confidence during calm periods');
          }
        }

        print('✅ EPISODE ANALYSIS COMPLETE');
        print('📊 System successfully tracked anxiety progression from calm → panic → recovery');
