...
async function testNotificationsWithBetterDesign() {
  ...
  try {
    ...
    if (!userData?.fcmToken) {
      console.log(
        "❌ No FCM token found. Please run your Flutter app first to register FCM token."
      );
      return;
    }
    ...
    const { error: deleteError } = await supabase
      ...
      .eq("user_id", userId)
      .or(
        "title.ilike.%test%,title.ilike.%demo%,title.ilike.%🔴%,title.ilike.%💚%,title.ilike.%🫁%"
      );
    ...
    console.log(
      "✅ Anxiety alert sent! ID:",
      anxietyResponse.substring(anxietyResponse.lastIndexOf("/") + 1)
    );
    ...
    console.log(
      "✅ Wellness check sent! ID:",
      wellnessResponse.substring(wellnessResponse.lastIndexOf("/") + 1)
    );
      ...
      .insert([
        {
          ...
          title: "🚨 Anxiety Alert",
          message:
            "Elevated heart rate detected (105 BPM). Your wellness is our priority - take deep breaths and find a calm space.",
          type: "alert",
          read: false,
          related_screen: "breathing_screen",
          related_id: null,
      ...
      .insert([
        {
          ...
          title: "💙 Daily Wellness Check",
          message:
            "Take a moment to reflect on your feelings today. Your mental health journey matters, and we're here to support you.",
          type: "reminder",
          read: false,
          related_screen: "wellness_check",
          related_id: null,
      ...
      .insert([
        {
          ...
          message:
            "A few minutes of focused breathing can help calm your mind and reduce stress. Try the 4-7-8 technique.",
          type: "reminder",
          read: false,
          related_screen: "breathing_screen",
          related_id: null,
    ...
    console.log(
      "2. 🏠 In-app notifications appear on homepage when app is OPEN"
    );
    console.log("3. 📋 All notifications are stored in Supabase database");
    console.log(
      "4. 🔄 Opening app after receiving push notifications will show them in-app"
    );
    console.log(
      "5. ⏰ Push notifications may not appear if app was recently opened (Android behavior)"
    );

    console.log("\n🧪 TESTING INSTRUCTIONS:");
    console.log("========================");
    console.log(
      "1. 📱 Close your Flutter app COMPLETELY (force close from recents)"
    );
    console.log("2. ⏳ Wait 10 seconds for app to fully close");
    console.log("3. 🔄 Run this script again to send new push notifications");
    console.log(
      "4. 📲 Check your device notification tray for push notifications"
    );
