  ...
  try {
    for (const appointment of testAppointments) {
      ...
      if (error) {
        ...
        console.log(`✅ Created test appointment: ${data[0].id} (${appointment.request_message})`);
      }
    }
...
async function checkExpiredAppointments() {
  ...
  try {
    ...
    expiredAppointments.forEach((apt, index) => {
      ...
      console.log(`      Response: ${apt.response_message || 'No response message'}`);
      console.log('');
    });
  } catch (error) {
...
async function triggerManualExpiration() {
  ...
  try {
    ...
    console.log(`📋 Found ${overdueAppointments.length} appointments to expire`);
    ...
    for (const appointment of overdueAppointments) {
      ...
      if (updateError) {
        console.error(`❌ Error expiring appointment ${appointment.id}:`, updateError);
      } else {
...
async function cleanupTestAppointments() {
  ...
  try {
    const { data, error } = await supabase
      .from('appointments')
      .delete()
      .or('user_id.eq.test-user-1,user_id.eq.test-user-2,user_id.eq.test-user-3')
      .select();
