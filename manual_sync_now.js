...
async function manualSyncFromSupabase() {
  ...
  try {
    ...
    const currentFirebaseRef = db.ref(
      `/devices/${CURRENT_SUPABASE_ASSIGNMENT.device_id}/assignment`
    );
    ...
    console.log(
      `   Firebase User: ${
        currentFirebase ? currentFirebase.assignedUser : "None"
      }`
    );
    ...
    console.log(
      `✅ User baseline synced: ${CURRENT_SUPABASE_ASSIGNMENT.baseline_hr} BPM`
    );

    // Initialize new user session
    await db
      .ref(
        `/users/${CURRENT_SUPABASE_ASSIGNMENT.user_id}/sessions/${newSessionId}/metadata`
      )
      .set({
        deviceId: CURRENT_SUPABASE_ASSIGNMENT.device_id,
        status: "active",
